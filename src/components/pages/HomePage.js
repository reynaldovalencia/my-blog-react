...
const HomePage = () => {
    ...
    const [state, dispatch] = useReducer(reducer, { loading: false, error: "", posts: [], loadingUsers: false, errorUsers: "", users: [], user: {} });
    ...
    const loadPosts = async () => {
        ...
        try {
            ...
            const filteredPost = query ? data.filter(x => x.title.indexOf(query) >= 0 || x.body.indexOf(query) >= 0) : data;
    ...
    const loadUsers = async () => {
        ...
        try {
            const { data } = await axios.get(userId ? `${backendAPI}/users/${userId}`
                : `${backendAPI}/users`);
    ...
    useEffect(() => {
        loadPosts();
        loadUsers();
    }, [query, userId, backendAPI])   //Cuando hay cambio en query se actualiza useEffect
